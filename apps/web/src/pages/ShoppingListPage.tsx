...
export function ShoppingListPage() {
  ...
  if (items.length === 0) {
    return (
      <div className="py-16 text-center">
        ...
        <h1 className="mt-4 text-2xl font-bold">Einkaufsliste ist leer</h1>
        <p className="mt-2 text-gray-500">
          Füge Produkte hinzu, um Preise zu vergleichen.
        </p>
        <Link
          to="/"
  ...
  return (
    <div className="space-y-6">
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold">Preisvergleich</h2>
        {loading ? (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-1 text-sm text-gray-500">
                      {store.items_available} von {store.items_available + store.items_missing} Produkten verfügbar
                    </div>
                    {store.missing_products.length > 0 && (
                      <div className="mt-1 text-xs text-red-500">
                        Nicht verfügbar: {store.missing_products.join(", ")}
                      </div>
                    )}
                  </div>
                );
            ...
            {totals.recommendation.max_savings_cents > 0 && (
              <div className="rounded-lg bg-green-50 p-3 text-center text-sm text-green-800">
                <span className="font-medium">
                  Du sparst bis zu {totals.recommendation.max_savings_formatted}
                </span>
